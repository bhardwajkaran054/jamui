...
import { Search, Package, Clock, CheckCircle2, XCircle, User, Phone, ShoppingBag } from 'lucide-react'
import { apiFetch } from '../api'
import type { OrderTrackingProps } from '../types'
import type { Order } from '../types'

export default function OrderTracking({ initialOrderId, onClose }: OrderTrackingProps) {
  ...
  const handleSearch = async (query?: string) => {
    ...
    try {
      let numericId = searchVal.replace(/[^0-9]/g, '')
      if (!numericId) {
      ...
      const result = await apiFetch<Order>(`/track/${numericId}`)
      setOrder(result)
    } catch (err) {
