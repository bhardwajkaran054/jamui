...
export default function AdminDashboard({ token, onLogout, onAdminAction, products, categories, orders: initialOrders }: AdminDashboardProps) {
  ...
  const [notices, setNotices] = useState<Notice>({ text: '', active: false })
  const [deliveryZones, setDeliveryZones] = useState<DeliveryZone[]>([])
  const [drivers, setDrivers] = useState<Driver[]>([])
  const [settings, setSettings] = useState<SettingsType>({ publicOrderToken: '' })
  ...
  const handleUpdateNotice = async (newNotice: Notice) => {
    try {
                  ...
                  <div className="bg-gray-50 p-6 rounded-3xl">
                    ...
                    <div className="flex items-end gap-3 h-32">
                      {[...Array(6)].map((_, i) => {
                        ...
                        const height = Math.min((monthTotal / (maxTotal * 5)) * 100, 100)
                        return (
                                    ...
                                    {item.image ? (
                                      <img
                                        ...
                                        onError={(e) => {
                                          ...
                                          e.currentTarget.parentElement!.innerHTML = `<span class="text-2xl">${item.emoji}</span>`;
                                        }}
                                      />
                                    ) : (
          ...
          <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-500">
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm space-y-8">
              <div className="space-y-4">
                ...
                <textarea
                  value={notices.text}
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNotices({ ...notices, text: e.target.value })}
                ...
                <tbody className="divide-y divide-gray-50">
                  {stockLogs.map(log => (
                    <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="p-6 font-black text-gray-800">{log.product_name}</td>
                      <td className="p-6 font-bold text-gray-400">{log.old_stock}</td>
                      <td className="p-6 font-black text-gray-900">{log.new_stock}</td>
                      <td className="p-6">
                        <span className={`px-2 py-1 rounded-lg text-[10px] font-black ${
                          log.new_stock > log.old_stock ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                        }`}>
                          {log.new_stock > log.old_stock ? '+' : ''}{log.new_stock - log.old_stock}
                        </span>
                      </td>
                    ...
                    <div className="bg-gray-50 w-16 h-16 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-inner overflow-hidden relative">
                      {product.useImage && product.image ? (
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.onerror = null;
                            e.currentTarget.parentElement!.innerHTML = `<span class="text-3xl">${product.emoji}</span>`;
                          }}
                        />
                      ) : (
